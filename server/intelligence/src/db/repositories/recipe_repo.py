...
Covers recipes, recipe_food_items, and food_items tables.
"""
...
async def get_recipe_with_food_items(
    pool: asyncpg.Pool,
    recipe_id: str,
) -> dict[str, Any] | None:
    """Fetch a recipe joined with its food items and per-100g nutrition.

    Returns the recipe dict with a nested ``food_items`` list, each
    containing food item details and ``grams_equivalent``.
    """
    async with pool.acquire() as conn:
        ...
        food_item_rows = await conn.fetch(
            """
            SELECT ri.food_item_id,
                   ri.grams_equivalent,
                   ri.notes,
                   ...
                   i.fat_per_100g,
                   i.fiber_per_100g,
                   COALESCE(fc.cost_per_100g, 0.00) AS estimated_cost_per_100g
    ...
    result["food_items"] = [dict(r) for r in food_item_rows]
    return result
...
async def calculate_recipe_cost(
    ...
    """Calculate the total food item cost for a recipe.

    Cost is derived from ``cost_per_100g`` on ``raw_food_costs`` scaled by
    ``grams_equivalent`` for each food item.
    """
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT COALESCE(
                       SUM(COALESCE(fc.cost_per_100g, 0.00) * ri.grams_equivalent / 100.0),
                       0
                   ) AS total_cost
              FROM recipe_food_items ri
              JOIN food_items i ON ri.food_item_id = i.id
...
async def get_recipe_allergen_food_items(
    pool: asyncpg.Pool,
    recipe_id: str,
    allergen_names: list[str],
) -> list[dict[str, Any]]:
    """Find food items in a recipe that match any of the given allergens.

    Matching is case-insensitive via ``ILIKE`` against the food item name.
    """
